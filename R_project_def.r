...
# Group: Cardini Marta, Corti Magalì, Schlag Louis
...
?aravo
# dataset of the distribution of 82 species of Alpine plants in 75 sites in Aravo (Valloire, France).
# aravo is a data.frame of the 82 species (columns)  in the 75 sites (rows). 
# aravo_env is a data.frame with the measurements of 6 environmental variables for the sites.
...
observations <- nrow(aravo)            # number of observation = number of rows = 7
species <- ncol(aravo)                 # number of species = number of columns of the community matrix = 86
env_variables <- ncol(aravo_env)       # number of environmental variables = number of columns of the environmental variables dataframe = 6


# checking data structure
str(aravo)
# data.frame of 75 obs. of  82 variables
# the variables assume integer values
str(aravo_env)
# data.frame of 75 obs. of  6 variables
# 5 variables are integer data types, while 1 variable is a character data type


# transforming columns to match types and structures described in the documentation (categorical variables)
...
aravo <- aravo[-index_na, ]      # omitting the corresponding rows in aravo, using index_na
# now we can see the number of rows (observations) has changed and is equal to 73

# making summary statistics
summary(aravo_env)
# information about  Min - 1st Qu. - Median - Mean - 3rd Qu. - Max of the numerical variables
# information about total number of observations for each levels of the categorical variables
...
hist(aravo_env$Aspect,
     ...
     xlab = "Aspect")
# main - argument to change the main title
# xlab - argument to change the name of the x axis
...
# to construct a barplot we have to make a contingency table with the function table() to count the occurrences of one class in the dataset
...
hist(aravo_env$Snow,
     ...
     xlab = "Mean snowmelt date (Julian day)")
dev.off()
# using width, height and res arguments to change the size and resolution of the final exported picture
# using dev.off() to stop the png exporting function 
...
boxplot(sr ~ Form,
        ...
        ylim = c(0,30))
# ylab - argument to change the name of the y axis
# ylim - argument to set the limits for the y axis, to better visualize the boxplot
...
plot(sr ~ Aspect,
     ...
     ylim = c(0,30),
     cex = 0.5)
# cex - argument to change the points' size on the plot (0.5 = 50% smaller compared to the default size)
